...
func New(logger *slog.Logger, routerOptions ...Option) http.Handler {
	...
	if configured.db != nil {
		...
		mux.Handle("DELETE /api/v1/marketplace/apps/{slug}/reviews/me", middleware.JWT(configured.auth.JWTSecret)(http.HandlerFunc(authHandler.DeleteReview)))
		...
		mux.Handle("GET /api/v1/me/favorites", middleware.JWT(configured.auth.JWTSecret)(http.HandlerFunc(authHandler.ListMyFavorites)))
